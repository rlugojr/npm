...
var runEnv = require('./utils/run-env.js')
...
function filterByEnv (data) {
  ...
  Object.keys(data.dependencies).forEach(function (name) {
    ...
    if (runEnv.onlyProd && !runEnv.onlyDev && keys.indexOf(name) !== -1) return
    if (runEnv.onlyDev && !runEnv.onlyProd && keys.indexOf(name) === -1) return
    if (!runEnv.onlyDev && keys.indexOf(name) !== -1 && data.dependencies[name].missing) return
