...
function thenRequireSafe (idealTree, next) {
  validate('OF', arguments)
  var options = {package: idealTree.package, offline: false}
  if (npm.config.get('shrinkwrap') && idealTree.package._shrinkwrap) {
    options.shrinkwrap = idealTree.package._shrinkwrap
  }
  log.silly('requireSafe', 'requesting')
  requireSafe.check(options, pulseTillDone('requireSafe', function (er, result) {
    if (er) {
      idealTree.warnings.push(er)
      return next()
    }
    result.forEach(function (finding) {
      var warnObj = new Error(finding.path.join(' > ') + '@' + finding.version + ': ' + finding.title + ' ' + finding.advisory)
      warnObj.code = 'EVULNERABLE'
      idealTree.warnings.push(warnObj)
    })
    next()
  }))
